...
export async function POST(req: NextRequest) {
  try {
    ...
    const privateKey = metadata?.custom_fields.master_wallet.private_key;
    const rpc = metadata?.custom_fields.ids_stake_wallet.rpc_url;
    const chain_id = metadata?.custom_fields.ids_stake_wallet.chain_id;
