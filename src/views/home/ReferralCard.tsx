...
export function ReferralSection({
  ...
  const copyReferralLink = async () => {
    try {
      await navigator.clipboard.writeText(
        `${process.env.NEXT_PUBLIC_WEB_URL}${registrationUsername || account?.username}`
      );
      setCopied(true);
  ...
  // Nếu đang loading hoặc chưa kết nối ví thì chỉ hiển thị giao diện kết nối ví
  if (loading || !isConnected || !account) {
    return (
        ...
        <CardContent className="space-y-4">
          <div className="p-6 bg-gray-800 rounded-lg border border-yellow-600/50">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="registration-email" className="text-white font-semibold">
                  Email
                </Label>
  ...
  // 2. Nếu đã đăng ký (có username) nhưng chưa vip
  if ((account as any).username && !(account as any).isVip) {
    return (
      <Card className="bg-gray-900 border-gray-800">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="secondary"
                className="bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-gray-200"
              >
                {t("referral.free")}
              </Badge>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Thông tin bạn bè và hoa hồng có thể không có nếu chưa là vip, nên ẩn hoặc hiển thị mặc định 0 */}
          <div className="grid grid-cols-2 gap-3">
            <div className="text-center p-3 bg-gray-800 rounded-lg border border-gray-700">
              <div className="text-lg font-bold text-blue-400">{formatNumber((account as any).f1 ?? 0, { decimal: false })}</div>
              <div className="text-xs text-gray-400">
                {t("referral.friendsJoined")}
              </div>
            </div>
            <div className="text-center p-3 bg-gray-800 rounded-lg border border-gray-700">
              <div className="text-lg font-bold text-emerald-400">${formatNumber((account as any).commission?.all ?? 0)}</div>
              <div className="text-xs text-gray-400">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-emerald-400">+${formatNumber((account as any).commission?.day ?? 0)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-300">{t("referral.monthlyEarnings")}</span>
              <span className="text-emerald-400">+${formatNumber((account as any).commission?.month ?? 0)}</span>
            </div>
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-gray-800 rounded-lg border-2 border-dashed border-blue-600/50">
            <div className="text-center mb-3">
              ...
              <div className="text-xs text-gray-400">
                {t("referral.earn", { percent: "50%" })}
              </div>
            </div>

            <div className="flex items-center space-x-2">
              <Input
                value={`${process.env.NEXT_PUBLIC_WEB_URL}${acc.username}`}
                readOnly
                className="text-xs bg-gray-700 border-gray-600 text-gray-300"
              />
              <Button
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="text-center p-3 bg-gray-800 rounded-lg border border-gray-700">
              <div className="text-lg font-bold text-blue-400">{formatNumber(acc.f1 ?? 0, { decimal: false })}</div>
              <div className="text-xs text-gray-400">
                {t("referral.friendsJoined")}
              </div>
            </div>
            <div className="text-center p-3 bg-gray-800 rounded-lg border border-gray-700">
              <div className="text-lg font-bold text-emerald-400">
                ${formatNumber(acc.commission?.all ?? 0)}
              </div>
              <div className="text-xs text-gray-400">
                {t("referral.usdtEarnings")}
              </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-emerald-400">
                +${formatNumber(acc.commission?.day ?? 0)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              ...
              <span className="text-emerald-400">
                +${formatNumber(acc.commission?.month ?? 0)}
              </span>
            </div>
          </div>
